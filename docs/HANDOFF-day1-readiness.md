...
4. ~~**Document-scanner gap:** appointments captured by the document scanner land in `schedule_tasks`, not `medical_appointments` — so they never generate a night-before reminder call.~~ **Fixed 2026-08-10.** `POST /documents/apply` (`documents.ts`) now writes each scanned appointment into `schedule_tasks` (dashboard/rotation display) **and** `medical_appointments` (the reminder job's data source) independently, inferring a `type` the fasting-check keywords in `call-scheduler.ts` still recognize (blood/lab → `lab_work`). The existing duplicate/already-applied checks were decoupled per-table so re-applying a document via "Update Care Plan" backfills whichever table is missing a row, without duplicating the other. **Decision point, left manual on purpose:** appointments from documents applied *before* this fix exist only in `schedule_tasks` and are not bulk-migrated by this change — Ray must re-apply that specific document ("Update Care Plan") or re-enter the appointment by hand in the Appointments tab to get it into `medical_appointments`.

...
const NAV_ITEMS = [
  ...
  { path: "/intercom", label: "Intercom", icon: <MessageSquare size={20} /> },
...
function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border flex items-center">
      ...
      <button
        onClick={lock}
        className="flex-none px-4 flex flex-col items-center gap-1 py-3 text-xs font-display uppercase tracking-wider text-muted-foreground hover:text-destructive transition-colors border-l border-border"
        title="Lock vault"
      >
        <Lock size={20} />

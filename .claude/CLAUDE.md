...
5. **Intercom is GONE** (was: "E2EE intercom, ciphertext/iv/salt only"). The intercom page, its API route, and `crypto.ts` were deliberately deleted in `61c5421` ("Remove encryption and vault features"). (The `Knowledgebase/` pre-removal snapshot was deleted in the 2026-08-20 repo sweep; it survives only in git history.) Don't wire it back in. (The *vault gate* is separate and very much still live — see Vault Gate above.)
